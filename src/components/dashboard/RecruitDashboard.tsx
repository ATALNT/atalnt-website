...
import { fetchRecruitJobs, fetchRecruitApplications, fetchRecruitCandidates } from '@/lib/dashboard-api';
import {
...
export function RecruitDashboard({ token, datePreset, dateRange }: RecruitDashboardProps) {
  ...
  const candidatesQuery = useQuery({
    queryKey: ['recruit', 'candidates', datePreset],
    queryFn: () => fetchRecruitCandidates(token, dateRange.from, dateRange.to),
    refetchInterval: 5 * 60 * 1000,
    staleTime: 2 * 60 * 1000,
  });
  ...
  // Candidate data: Form Data (Recruiter field filled) vs Operations (direct Zoho entry)
  const formRecruiters = candidatesData?.formData || [];
  const formTotal = candidatesData?.formTotal || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ============================================ */}
      {/* New Candidates — Form Data + Operations      */}
      {/* ============================================ */}
      {formRecruiters.length > 0 && (
        <div>
            ...
            <Card className="border-white/[0.06] bg-white/[0.02] backdrop-blur-sm relative overflow-hidden">
              ...
              <CardHeader className="pb-3 pt-5 px-5">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-xs font-semibold text-white/40 uppercase tracking-[0.15em] flex items-center gap-2">
                    <FileText className="h-4 w-4 text-emerald-400/60" />
                    Candidate Submission Form
                  </CardTitle>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-white/25">{periodLabel}</span>
                    <span className="text-sm font-bold text-emerald-400 bg-emerald-400/10 px-2.5 py-0.5 rounded-full">
                      {formTotal}
                    </span>
                  </div>
                </div>
              ...
              <CardContent className="px-5 pb-5">
                <div className="space-y-1">
                  {formRecruiters.map((r: any) => {
                    const maxCount = Math.max(...formRecruiters.map((s: any) => s.count));
                    const barWidth = maxCount > 0 ? Math.max((r.count / maxCount) * 100, 8) : 8;
                    const isExpanded = expandedRecruiter === `form-${r.recruiterName}`;
                    return (
                      <div key={r.recruiterName}>
                        <button
                          onClick={() => setExpandedRecruiter(isExpanded ? null : `form-${r.recruiterName}`)}
                          className="w-full text-left group"
                        >
                          <div className="flex items-center gap-3 py-1.5 px-2 -mx-2 rounded-lg hover:bg-white/[0.03] transition-colors">
                            <ChevronDown className={`h-3.5 w-3.5 text-white/25 transition-transform shrink-0 ${isExpanded ? 'rotate-180' : ''}`} />
                            <span className="text-sm font-medium text-white/70 group-hover:text-white/90 transition-colors w-28 shrink-0">
                              {r.recruiterName}
                            </span>
                            <div className="flex-1 h-2 bg-white/[0.04] rounded-full overflow-hidden">
                              <div
                                className="h-full bg-gradient-to-r from-emerald-500/80 to-emerald-400 rounded-full transition-all duration-700"
                                style={{ width: `${barWidth}%` }}
                              />
                            </div>
                            <span className="text-base font-bold text-emerald-400 w-8 text-right shrink-0">
                              {r.count}
                            </span>
                          </div>
                        </button>
                        {isExpanded && r.candidates && (
                          <div className="mt-1 mb-2 ml-8 mr-1 max-h-64 overflow-y-auto rounded-lg bg-white/[0.02] border border-white/[0.04]">
                            <table className="w-full text-xs table-fixed">
                              <thead>
                                <tr className="text-[10px] text-white/25 uppercase tracking-wider border-b border-white/[0.06]">
                                  <th className="text-left py-2 px-3 font-medium w-[22%]">Candidate</th>
                                  <th className="text-left py-2 px-3 font-medium w-[28%]">Job Opening</th>
                                  <th className="text-left py-2 px-3 font-medium w-[18%]">Client</th>
                                  <th className="text-left py-2 px-3 font-medium w-[17%]">Stage</th>
                                  <th className="text-left py-2 px-3 font-medium w-[15%]">Source</th>
                                </tr>
                              </thead>
                              <tbody>
                                {r.candidates.map((c: any, i: number) => (
                                  <tr key={i} className="border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors">
                                    <td className="py-1.5 px-3 text-white/60 truncate overflow-hidden">{c.name}</td>
                                    <td className="py-1.5 px-3 text-white/40 truncate overflow-hidden">{c.job}</td>
                                    <td className="py-1.5 px-3 text-white/35 truncate overflow-hidden">{c.client}</td>
                                    <td className="py-1.5 px-3 truncate overflow-hidden">
                                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                                        c.stage?.toLowerCase().includes('hired') ? 'bg-emerald-500/15 text-emerald-400' :
                                        c.stage?.toLowerCase().includes('offer') ? 'bg-amber-500/15 text-amber-400' :
                                        c.stage?.toLowerCase().includes('interview') ? 'bg-violet-500/15 text-violet-400' :
                                        c.stage?.toLowerCase().includes('submitted') ? 'bg-[#D4A853]/15 text-[#D4A853]' :
                                        c.stage?.toLowerCase().includes('reject') || c.stage?.toLowerCase().includes('unqualified') ? 'bg-red-500/15 text-red-400' :
                                        c.stage?.toLowerCase().includes('withdrawn') ? 'bg-purple-500/15 text-purple-400' :
                                        'bg-white/5 text-white/40'
                                      }`}>
                                        {c.stage || '—'}
                                      </span>
                                    </td>
                                    <td className="py-1.5 px-3 text-white/25 truncate overflow-hidden">{c.source}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
      ...
      {/* From Recruit Applications pipeline data      */}
      {/* ============================================ */}
      {submissionsByRecruiter.length > 0 && (
        <Card className="border-white/[0.06] bg-white/[0.02] backdrop-blur-sm relative overflow-hidden">
          ...
          <CardHeader className="pb-3 pt-5 px-5">
            <div className="flex items-center justify-between">
              <CardTitle className="text-xs font-semibold text-white/40 uppercase tracking-[0.15em] flex items-center gap-2">
                <Send className="h-4 w-4 text-[#D4A853]/60" />
                Submitted to Client
              </CardTitle>
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-white/25">{periodLabel}</span>
                <span className="text-sm font-bold text-[#D4A853] bg-[#D4A853]/10 px-2.5 py-0.5 rounded-full">
                  {submissionsByRecruiter.reduce((sum: number, r: any) => sum + r.submitted, 0)}
                </span>
              </div>
            </div>
          </CardHeader>
          <CardContent className="px-5 pb-5">
            <div className="space-y-1">
              {submissionsByRecruiter.filter((r: any) => r.submitted > 0).map((r: any) => {
                const filtered = submissionsByRecruiter.filter((s: any) => s.submitted > 0);
                const maxCount = Math.max(...filtered.map((s: any) => s.submitted));
                const barWidth = maxCount > 0 ? Math.max((r.submitted / maxCount) * 100, 8) : 8;
                const isExpanded = expandedRecruiter === `sub-${r.recruiterName}`;
                return (
                  <div key={r.recruiterName}>
                    <button
                      onClick={() => setExpandedRecruiter(isExpanded ? null : `sub-${r.recruiterName}`)}
                      className="w-full text-left group"
                    >
                      <div className="flex items-center gap-3 py-1.5 px-2 -mx-2 rounded-lg hover:bg-white/[0.03] transition-colors">
                        <ChevronDown className={`h-3.5 w-3.5 text-white/25 transition-transform shrink-0 ${isExpanded ? 'rotate-180' : ''}`} />
                        <span className="text-sm font-medium text-white/70 group-hover:text-white/90 transition-colors w-32 shrink-0">
                          {r.recruiterName}
                        </span>
                        <div className="flex-1 h-2 bg-white/[0.04] rounded-full overflow-hidden">
                          <div
                        ...
                        <span className="text-[11px] text-white/25 shrink-0">{r.newInPeriod} cand</span>
                        <span className="text-base font-bold text-[#D4A853] w-8 text-right shrink-0">
                          {r.submitted}
                        </span>
                      </div>
                    </button>
                    {isExpanded && r.candidates && (
                      <div className="mt-1 mb-2 ml-8 mr-1 max-h-64 overflow-y-auto rounded-lg bg-white/[0.02] border border-white/[0.04]">
                        <table className="w-full text-xs table-fixed">
                          <thead>
                            <tr className="text-[10px] text-white/25 uppercase tracking-wider border-b border-white/[0.06]">
                              <th className="text-left py-2 px-3 font-medium w-[20%]">Candidate</th>
                              <th className="text-left py-2 px-3 font-medium w-[25%]">Job Opening</th>
                              <th className="text-left py-2 px-3 font-medium w-[18%]">Client</th>
                              <th className="text-left py-2 px-3 font-medium w-[12%]">Recruiter</th>
                              <th className="text-left py-2 px-3 font-medium w-[25%]">Status</th>
                            </tr>
                          </thead>
                          <tbody>
                            {r.candidates.map((c: any, i: number) => (
                              <tr key={i} className="border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors">
                                ...
                                <td className="py-1.5 px-3 text-white/40 truncate overflow-hidden">{c.job}</td>
                                <td className="py-1.5 px-3 text-white/35 truncate overflow-hidden">{c.client}</td>
                                <td className="py-1.5 px-3 text-emerald-400/50 truncate overflow-hidden">{c.recruiter || '—'}</td>
                                <td className="py-1.5 px-3 truncate overflow-hidden">
                                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                                    c.status?.toLowerCase().includes('hired') ? 'bg-emerald-500/15 text-emerald-400' :
                                    c.status?.toLowerCase().includes('offer') ? 'bg-amber-500/15 text-amber-400' :
                                    c.status?.toLowerCase().includes('interview') ? 'bg-violet-500/15 text-violet-400' :
                                    c.status?.toLowerCase().includes('submitted') ? 'bg-[#D4A853]/15 text-[#D4A853]' :
                                    c.status?.toLowerCase().includes('reject') || c.status?.toLowerCase().includes('unqualified') ? 'bg-red-500/15 text-red-400' :
                                    c.status?.toLowerCase().includes('withdrawn') ? 'bg-purple-500/15 text-purple-400' :
                                    'bg-white/5 text-white/40'
                                  }`}>
                                    {c.status || '—'}
                                  </span>
